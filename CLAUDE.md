# nvme-cli-ai — Claude Context

## About this repository

nvme-cli-ai is a companion repository providing optional AI-assisted development
resources for nvme-cli and the broader linux-nvme ecosystem. It is:

- **optional** — not required to contribute to nvme-cli
- **tool-neutral** — contributors are not assumed to use any specific AI tool
- **non-invasive** — no changes to the main nvme-cli repository are required
...
```
workspace/
├── nvme-cli/       ← main project (source, build, tests, git history)
└── nvme-cli-ai/    ← this repo (AI context, rules, prompts)
```

When working on nvme-cli from within this repository, navigate to `../nvme-cli/`
for all source edits, builds, and git operations.

## nvme-cli-ai structure

```
CLAUDE.md               This file — loaded automatically by Claude Code
.gitignore              Prevents personal Claude state from being committed
.claude/
...
| Commit to nvme-cli-ai | Keep local / never commit |
|-----------------------|--------------------------|
| `CLAUDE.md` | `CLAUDE.local.md` |
| `.claude/settings.json` | `.claude/settings.local.json` |
| `.claude/commands/` | `~/.claude/` (global Claude state) |
| `.claude/rules/` | |

---

# nvme-cli — Project Reference Guide

> All file paths, build commands, and git operations in this section refer to
> the **nvme-cli** repository at `../nvme-cli/` relative to this repo.

## Project overview

nvme-cli is the Linux command-line utility for NVM-Express (NVMe) SSDs. As of
version 3.x, the libnvme library is fully integrated into this repository (not
an external dependency, not a git submodule — the complete source lives in
...
Run all build commands from `../nvme-cli/`:
...
12 GitHub Actions workflows in `../nvme-cli/.github/workflows/`. Key ones:
...
All paths are relative to `../nvme-cli/`.
